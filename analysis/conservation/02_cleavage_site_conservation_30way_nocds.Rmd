...
gr_cleavage %<>% 
    `[`(idx_valid) %>%
    mutate(score=gscores(x=phastcons_hg38, unlist(grl_diff[idx_valid]), scores.only=TRUE))
```
...
ggsave("img/sq/fig1e-hg38-phastcons30-main.pdf", width=3, height=4, dpi=300)
```
...
ggsave("img/sq/fig1e-hg38-phastcons30-som3.pdf", width=4, height=4, dpi=300)
```
...
ggsave("img/sq/fig1e-hg38-phastcons30-som2.pdf", width=4, height=4, dpi=300)
```
...
```{r plt_n_celltypes_ip_count, fig.width=10, fig.height=4}
df_combined %T>% {
        df_counts <<- count(., origin_split, n_celltypes_bin, name="n_genes")
    } %>%
    ggplot(aes(x=origin_split, y=score, fill=n_celltypes_bin)) +
